...
const Examination = () => {
    ...
    const handleSubmit = (ev) => {
        ...
        var resp;
        apiRequest(`http://${window.location.hostname}/api/v1/doctors/create_examination`, requestOptions)
          .then(
            (response) => {
                resp = response;
    ...
    <form id="survey-form" ref={formRef} onSubmit={handleSubmit} action="">
        <div class="group">
            <SearchUser 
                ...
                url={`http://${window.location.hostname}/api/v1/doctors/search_user/?username=`} 
                searchBy="username"
                inputId={patientId}
