...
export function SearchExamination({inputFieldLabel, searchBy, user, setUser, inputId, setInputId, examinationResults, setExaminationResults}) {
    ...
    const searchUser = search_user(`http://${window.location.hostname}/api/v1/doctors/search_user/?username=`, setSearchResults, setInputId, setExaminationResults)

    const getUserExaminations = debounce((event) => {
        apiRequest(`http://${window.location.hostname}/api/v1/doctors/user_examinations/?user_id=${event.target.id}`, requestOptions)
            .then(res => res.json())
            .then(
            (result) => {
