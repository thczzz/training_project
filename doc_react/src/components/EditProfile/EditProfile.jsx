...
const EditProfileForm = () => {
    ...
    function getAndSetUserInfoEditProfile () {
        ...
        var resp;
        fetch(`http://${location.hostname}:3000/api/v1/user_info`, {...requestOptions, credentials: 'include'})
          .then(
            (response) => {
                resp = response;
    ...
    const handleSubmit = (ev, props) => {
        ...
        var resp;
        apiRequest(`http://${location.hostname}:3000/api/v1/users`, requestOptions)
          .then(
            (response) => {
                resp = response;
                return response.json();
